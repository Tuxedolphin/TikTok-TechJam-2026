...
### Task 1: `EvalCase` type, RunEventType additions, and Database v4 migration

**Files:**
- Modify: `apps/server/src/types.ts` (RunEventType union at lines 46-60, `Database` at lines 105-113)
- Modify: `apps/server/src/store.ts` (`emptyDatabase` at lines 6-14, `migrateDatabase` at lines 16-80)
- Test: `apps/server/src/store.test.ts` (append a new describe block)

**Interfaces:**
- Consumes: nothing new (baseline types).
- Produces: `EvalCase` interface (verbatim from spec), `RunEventType` members `"eval.captured" | "eval.replayed" | "budget.anomaly" | "budget.degraded"`, `Database` with `version: 4` and `evalCases: EvalCase[]`. Later tasks read/write `database.evalCases`.

- [ ] **Step 1: Write the failing migration test**

Append to `apps/server/src/store.test.ts` (merge imports with what the file already has — it already imports `JsonStore`; add `mkdtemp`, `writeFile`, `path`, `tmpdir` if not present):
...
describe("Database v4 migration (evalCases)", () => {
  it("migrates a v3 database to v4 with an empty evalCases collection", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "store-v4-test-"));
    ...
    expect(store.snapshot().evalCases).toEqual([]);
  });

  it("preserves evalCases on a v4 database round-trip", async () => {
    const root = await mkdtemp(path.join(tmpdir(), "store-v4-keep-"));
    const filePath = path.join(root, "db.json");
    const store = new JsonStore(filePath);
    await store.initialize();
    await store.mutate((database) => {
      database.evalCases.push({
        id: "eval-1",
        sourceRunId: "run-1",
        agentId: "agent-1",
        prompt: "p",
    ...
    expect(reopened.snapshot().evalCases).toHaveLength(1);
  });
});
```
...
Run: `npm test -w @launchpad/server -- src/store.test.ts`
Expected: FAIL — typecheck/assert errors (`evalCases` does not exist on `Database`, version is 3).

- [ ] **Step 3: Add the types**

In `apps/server/src/types.ts`, extend the `RunEventType` union (after `"step.approval_denied"`):
...
Add the spec's `EvalCase` verbatim (place after `ApprovalRequest`):
...
- [ ] **Step 4: Update the store migration**

In `apps/server/src/store.ts` replace `emptyDatabase` and `migrateDatabase`:
...
// WS-A coordination point: WS-A adds principals/grants/resources to this same
// v4 migration. If its version-4 branch already exists, fold evalCases into it.
...
(The v1/v2 branch above is the file's existing session-backfill logic verbatim — the only changes from today's code are `version: 4` in the return literal and the added `evalCases: []`. The existing `randomUUID` import at the top of `store.ts` already covers it.)

- [ ] **Step 5: Run tests + typecheck to verify they pass**

Run: `npm test -w @launchpad/server -- src/store.test.ts && npm run typecheck`
Expected: PASS (all store tests, whole-repo typecheck).

- [ ] **Step 6: Commit**

```bash
git add apps/server/src/types.ts apps/server/src/store.ts apps/server/src/store.test.ts
git commit -m "feat(trace-intel): add EvalCase type, eval/anomaly run events, store v4 migration"
```
