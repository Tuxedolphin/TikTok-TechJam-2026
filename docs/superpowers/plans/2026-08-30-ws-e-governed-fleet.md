...
`Grant`, `GrantScope`, `FleetTopic`, `FleetTurn`, the `fleet.turn` / `fleet.timeout` `RunEventType`
members, and the `Database` v4 migration (including `fleetTopics: []` and `fleetTurns: []`) all landed
with WS-A Task 1 (commit `3c25d5e`). This task's original contents re-declared those types (duplicate
identifier errors) and its Step 4 replaced `emptyDatabase`/`migrateDatabase` wholesale, deleting WS-A's
working migration and seeding. Its "expected: FAIL" step would have silently passed.

**Do this instead — verification only (no edits):**

- [ ] Confirm `FleetTopic` and `FleetTurn` exist in `apps/server/src/types.ts` and match the spec.
- [ ] Confirm `RunEventType` includes `fleet.turn` and `fleet.timeout`.
- [ ] Confirm `Database` is `version: 4` with `fleetTopics` and `fleetTurns`.
- [ ] Run `npm run test -w @launchpad/server -- store.test.ts` — expected PASS (no changes made).
