...
export function loadConfig(environment: Record<string, unknown> = process.env) {
  ...
  if (modelProvider === "gemini") {
    modelApiKey = env.GEMINI_API_KEY?.trim() ?? "";
    modelName = env.GEMINI_MODEL?.trim() || "gemini-3.5-flash-lite";
    modelBaseUrl = "http://host.docker.internal:" + env.PORT + "/api/adapter";
  ...
  return {
    ...
    geminiApiKey: modelProvider === "gemini" ? modelApiKey : "",
    openRouterApiKey: modelApiKey,
